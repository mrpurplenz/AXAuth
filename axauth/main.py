...
def process_input(line, local_peer, message_stack, sessionmanager, signing_enabled):
    current_peer = sessionmanager.active_peer
    if line == "/exit":
        if sessionmanager:
        ...
        current_peer = None
        return "exit", message_stack, signing_enabled, current_peer
        ...
        try:
            ...
            sessionmanager.connect(local_peer, current_peer)
            message_stack.append(("info", f"[info] Connected to {current_peer}"))
        except ValueError:
            message_stack.append(("error",f"[error] Failed to connect to {next_current_peer}"))
            current_peer = sessionmanager.revert_last_active_peer()
        return "connection", message_stack, signing_enabled, current_peer
        ...
        return "signing_toggle", message_stack, signing_enabled, current_peer
    elif line and sessionmanager.has_active_peer():
        ...
        try:
            ...
            sessionmanager.send_packet(packet,signing_enabled)
            message_stack.append((message_status, f"[>{current_peer or 'unproto'}] {line}"))
        except BrokenPipeError:
            message_stack.append(("error",f"[error] Connection lost."))
            result = sessionmanager.dissconnect_session(remote_peer)
            current_peer = sessionmanager.revert_last_active_peer()
        return "text_sent_to_session", message_stack, signing_enabled, current_peer
    elif line:
        ...
        return "text_sent_as_unproto", message_stack, signing_enabled, current_peer

def run_peer_terminal(sessionmanager, local_call="N0CALL"):
    current_peer = None
    verified = False
    signing_enabled = True
    ...
    with term.fullscreen(), term.cbreak():
        ...
        while True:
            ...
            #print(term.hide_cursor(), end='', flush=True)
            #(signing_enabled, message_stack, log_stack, term, peer_list, sessionmanager)
            prompt_len, PROMPT_START_Y = render_terminal(signing_enabled, message_stack, log_stack, term, sessionmanager)
            #print(term.normal_cursor(), end='', flush=True)
            ...
            if process_flag:
                ...
                if sessionmanager.has_active_peer():
                    if signing_enabled:
                ...
                #Process the line
                result, message_stack, signing_enabled, current_peer = process_input(input_buffer, local_call,message_stack,sessionmanager,signing_enabled)
            ...
            if sessionmanager and sessionmanager.has_queue():
                while sessionmanager and sessionmanager.has_queue():
                    inbound_tuple = sessionmanager.get_one_inbound()
                    inbound_peer = inbound_tuple[0]
                    message_stack.append(inbound_tuple[1])
...
def dep_connect_to_peer(local_call, remote_call):
    try:
...
def main():
    ...
    if mode == 'peer':
        run_peer_terminal(sessionmanager,local_call)
    else:
